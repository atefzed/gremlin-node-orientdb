...
function eORv(coll, callback) {
    ...
    async.auto({
        ...
        final: ["v", "e", function(cb, results) {
            if (results.v && results.v.length !== 0)
                return cb(null, "v");
            else if (results.e && results.e.length !== 0)
                return cb(null, "e");
            else if ((!results.v || results.v.length === 0) && (!results.e || results.e.length === 0))
                return cb(null, "e");
        }]
    }, function(err, res) {
...
function Graph() {
            ...
            if (r === 'v') {
                ...
                async.auto({
                    ...
                    has: ["v", function(cb, results) {
                        if (filter === 'all') return cb(null);
                        if (filter !== '') coll = filter;
                        if (results.v) {
                            if (type === 'id') {
                    ...
                    if (filter === 'all') return callback(null, res.v[0]);
                    if (filter !== '') return callback(null, res.v[0]);
     ...
     * @return {Array of Objects} [description]
     */
    this.inV = function(conn, coll, obj, filter, callback) {
        var key, type, value, nbArgs = 0;
        for (key in obj) {
            type = key;
            value = obj[key];
        ...
        eORv(coll, function(e, r) {
            if (r === 'v') {
                async.auto({
                    v: function(cb) {
                        new Graph().v(conn, coll, obj, filter, function(err, record) {
                            if (err) return cb(err);
                        ...
                        if (results.v) {
                            ...
                            if (filter === 'all') {
                                if (type === 'id') {
                                    ...
                                    query("g.V('" + getType(type) + "', '" + value + "').in()");
                                client.fetch(query, function(err, response) {
                                    if (err) return cb(err);
                                    return cb(null, response);
                                });
                                ...
                                console.kig
                                if (type === 'id') {
                                    query("g.v('" + value + "').in().has('@class', '" + filter + "')");
                                } else
                                    query("g.V('" + getType(type) + "', '" + value + "').in().has('@class', '" + filter + "')");
                                client.fetch(query, function(err, response) {
                                    if (err) return cb(err);
                                    return cb(null, response);
                                });
    ...
    this.without = function(conn, coll, obj, arr) {
        if (obj instanceof Array && arr instanceof Array) {
            var arr1 = obj,
                size1 = arr1.length;
            var arr2 = arr,
                size2 = arr2.length;
        ...
        if (typeof obj === 'object' && arr instanceof Array) {
            var size = arr.length;
            for (var i = 0; i < size; i++) {
                if (obj._id === arr[i]._id) {
                    arr.splice(i, 1);
                    i--;
                    size--;
                }
        ...
        if (typeof obj === 'string' && arr instanceof Array) {
            var id = obj;
            var size = arr.length;
            for (var i = 0; i < size; i++) {
                if (id === arr[i]._id) {
     ...
     */
    this.dedup = function(conn, coll, obj) {
        if (!obj)
            throw new Error({
                name: 'VALIDATION',
                message: 'Some arguments are missing!'
     ...
     * @return {Number} [description]
     */
    this.count = function(conn, coll, obj) {
        if (!obj)
            throw new Error({
                name: 'VALIDATION',
                message: 'Some arguments are missing!'
        ...
        eORv(coll, function(e, r) {
            if (r === 'v') {
                ...
                if (!(arrIdVs instanceof Array))
                    return callback({
                        name: 'VALIDATION',
                        message: 'ARG2 must be an array of Ids!'
                    });
                if (typeof idV !== 'string' || typeof labelE !== 'string')
                    return callback({
                        name: 'VALIDATION',
                        message: 'ARG1 and ARG3 must be a strings!'
                    });
    ...
    this.each = function(conn, coll, obj, action, callback) {
        ...
        if (typeof action !== 'object')
            return callback('ARG2 must be an object!');
        var acc = [];
        if (obj instanceof Array) {
            async.eachSeries(obj, function(item, cb) {
                ...
                if (action.name === 'inV') {
                    if (item._type === 'vertex')
                        new Graph().inV(conn, coll, {
                            id: item._id
                        }, coll, function(err, res) {
                            if (err) return callback(err);
                            acc = merge(acc, res);
                            return cb();
                        });
                ...
                if (action.name === 'getByRelationship') {
                    if (item._type === 'vertex')
                        new Graph().bothByLabel(conn, coll, 'out', item._id, action.className, function(err, res) {
                            if (err) return callback(err);
                            item.friends = res;
                            return cb();
                        });
                ...
                if (err) return callback(err);
                if (acc.length != 0) return callback(null, acc);
                if (action.name === 'getByRelationship') return callback(null, obj);
            });
        } else {
            if (action === 'remove') {
                if (obj._type === 'vertex')
        ...
        async.auto({
            ...
            out: ['e', 'v', function(cb, results) {
                if (results.v && results.e) {
                    var acc = [],
                        arr = [];
                    new Graph().outE(conn, coll, {
                        "id": id
                    }, label, function(err, res) {
                        arr = res;
