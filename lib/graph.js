        ...
        eORv(coll, function(e, r) {
            if (r === 'v') {
                ...
                if (typeof type !== 'string' && typeof value !== 'string')
                    return callback({
                        name: 'VALIDATION',
                        message: 'ARG2 and ARG3 must be string!'
                    });
            ...
            if (r === 'v') {
                ...
                async.auto({
                    shortestP: function(cb) {
                        var query = gremlin();
                        console.log("g.v('" + idVStart + "').has('@class', '" + coll + "').out.loop(1){it.object.id != '" + idVEnd + "' && it.loops < '" + nbIterations + "'}.path");
                        query("g.v(%s).out.loop(1){it.object.id != %s && it.loops < %d}.path", idVStart, idVEnd, nbIterations);
