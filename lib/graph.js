 ...
 * @param {Functino} callback [description]
 * @return {String} [description]
 */
...
function Graph() {
    ...
    this.v = function(conn, coll, obj, filter, callback) {
        ...
        if (nbArgs > 1) {
            return callback({
                name: 'ERROR',
                message: 'ARG1 should take only one'
            });
        }
        if (!type && value === undefined && !filter && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
        }
        if (value === undefined && !filter && !callback) {
            callback = type;
    ...
    this.inV = function(conn, coll, obj, callback) {
        ...
        if (!type && value === undefined && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
    ...
    this.inE = function(conn, coll, obj, label, callback) {
        ...
        if (!type && value === undefined && !label && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
        }
        if (value === undefined && !label && !callback) {
            callback = type;
            type = value = label = undefined;
        } else if (!label && !callback) {
            callback = value;
    ...
    this.has = function(conn, coll, obj, type, callback) {
        ...
        if (!label && !type && value === undefined && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
    ...
    this.hasNot = function(conn, coll, label, value, type, callback) {
        ...
        async.auto({
            V: function(cb) {
                if (type === 'all' || type === 'vertex') {
                    if (typeof value == 'string')
                        query("g.V.has('@class', '" + coll + "').hasNot(%s, %s)", getType(label), value);
                    else
                        query("g.V.has('@class', '" + coll + "').hasNot(%s, %d)", getType(label), value);
                    client.execute(query, function(err, response) {
                        if (err) return cb({
                            name: 'ERROR',
                            message: err
            ...
            E: function(cb) {
                if (type === 'all' || type === 'edge') {
                    if (typeof value == 'string')
                        query("g.E.has('@class', '" + coll + "').hasNot(%s, %s)", getType(label), value);
                    else
                        query("g.E.has('@class', '" + coll + "').hasNot(%s, %d)", getType(label), value);
                    client.execute(query, function(err, response) {
                        if (err) return cb({
                            name: 'ERROR',
                            message: err
        ...
        eORv(coll, function(e, r) {
            if (r === 'v') {
                ...
                if (type !== 'in' && type !== 'out' && type !== 'all')
                    return callback({
                        name: 'VALIDATION',
                        message: 'TYPE must be in, out or all'
                    });
    ...
    this.linkBoth = function(conn, coll, idV, labelE, arrIdVs, callback) {
        if (!idV && !labelE && !arrIdVs && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
        ...
        eORv(coll, function(e, r) {
            if (r === 'v') {
                ...
                if (!(arrIdVs instanceof Array))
                    return callback({
                        name: 'VALIDATION',
                        message: 'ARG3 must be an array of Ids!'
                    });
                ...
                async.auto({
                        ...
                        e: function(cb) {
                            ...
                            if (results.e) {
                                ...
                                client.execute(query, function(err, response) {
                                    if (err === "Transaction was rolled back more times than it was started.")
                                        return cb({
                                            name: 'ERROR',
    ...
    this.ifThenElse = function(conn, coll, type, idV, condition, attTrue, attFalse, callback) {
        if (!type && !idV && !condition && !callback && !attFalse && !attTrue) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
                        ...
                        async.eachSeries(obj, function(item, cb) {
                            if (action === 'remove') {
                                if (item._type === 'vertex')
                                    new Graph().removeVertex(conn, coll, {
                                        'id': item._id
                                    }, function(err, res) {
                                        return cb(err, res);
                                    });
                                if (item._type === 'edge')
                                    new Graph().removeEdge(conn, coll, {
                                        'id': item._id
                                    }, cb);
                            }
                        }, function(err) {
    ...
    this.getLabels = function(conn, coll, id, callback) {
            ...
            async.auto({
                ...
                getP: ['v', 'e', function(cb, results) {
                    ...
                    if (results.v)
                        query("g.v('"+id+"').has('@class', '" + coll + "').outE.label");
                    if (!results.v && !results.e)
                        return cb({
                            name: 'ERROR',
        ...
        eORv(coll, function(e, r) {
            ...
            if (r === 'v') {
                ...
                async.auto({
                    ...
                    if (res.getP.results[0])
                        return callback(err, res.getP.results);
                    else return callback({
                        name: 'WARN',
                        message: 'This property doesn t exist in our model'
                    });
                });
            } else {
                return callback({
    ...
    this.shortestPath = function(conn, coll, idVStart, idVEnd, nbIterations, callback) {
        if (!idVStart && !idVEnd && !nbIterations && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
        ...
        if (!callback) {
            if (typeof idVStart == 'function') {
                callback = idVStart;
                idVStart = undefined
            }
            if (typeof idVEnd == 'function') {
                callback = idVEnd;
                idVEnd = undefined
            }
    ...
    this.loop = function(conn, coll, idV, condition1, condition2, callback) {
        if (!idV && !condition1 && !condition2 && !callback) {
            throw new Error({
                name: 'ERROR',
                message: 'Some arguments are missing!'
            });
